...
//    public static final int SYNC_INTERVAL = 60 * 180;
    ...
    private void notifyWeather() {
        ...
        if ( displayNotifications ) {
            ...
            if (System.currentTimeMillis() - lastSync >= DAY_IN_MILLIS) {
                ...
                Cursor cursor = context.getContentResolver().query(weatherUri, WeatherContract.NOTIFY_WEATHER_PROJECTION, null, null, null);

                if (cursor.moveToFirst()) {
                    int weatherId = cursor.getInt(WeatherContract.INDEX_WEATHER_ID);
                    double high = cursor.getDouble(WeatherContract.INDEX_MAX_TEMP);
                    double low = cursor.getDouble(WeatherContract.INDEX_MIN_TEMP);
                    String desc = cursor.getString(WeatherContract.INDEX_SHORT_DESC);
    ...
    private void notifyWeatherForWear() {
        Context context = getContext();
        //checking the last update and notify if it' the first of the day
        SharedPreferences prefs = PreferenceManager.getDefaultSharedPreferences(context);
...
//                            Log.d(LOG_TAG, "putDataItem result status: " + dataItemResult.getDataItem().getUri());
